...
model_path = r'D:\ComputerVisionProjectAI\RandomForestModel(LBP).pkl'
...
def preprocess_image(image):
    ...
    hist /= (hist.sum() + 1e-6)
    processed_image = hist
    
    return processed_image, lbp
...
# Show project intro and team details if the session_state is not set
if 'started' not in st.session_state or not st.session_state.started:
    st.markdown("""
        ...
        <div class="content">
            ...
            <p>This project provides two core functionalities: vehicle classification and segmentation.
            For classification, the application uses Local Binary Patterns (LBP) features and a Random Forest classifier to distinguish between vehicles, such as cars and bikes.
    ...
    if option == 'Classification':
        ...
        This application uses Local Binary Patterns (LBP) and a trained Random Forest model to classify images as either a car or a bike.
        """)
        ...
        if uploaded_file is not None:
            ...
            processed_image, lbp = preprocess_image(image)
            
            # Display HOG image in grayscale
            st.markdown("### HOG Feature Visualization")
            st.write("The grayscale image below shows the LBP features extracted from the uploaded image. These features are used by the model for prediction.")
            ...
            **Confidence Level:** {np.round(model.predict_proba(processed_image)[0][int(prediction[0])]*100, 2)}%
            ...
            st.markdown("""
            ### About the Model
            The model uses a **Random Forest Classifier** trained on LBP features to distinguish between cars and bikes. This type of classifier works by constructing multiple decision trees and outputting the class prediction that represents the majority vote of the trees.
            """)
